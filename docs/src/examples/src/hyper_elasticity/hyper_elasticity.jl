# # Total-lagrangian hyper-elasticity 
# ![NH](Neo-Hookean_Tensile_Test.png)
...
# ## Physics
# The script should be very intuitive without any trick. Only note MetaFEM is able to do symbolic differentiation with respect to an intermediate tensor variable by the built-in function `d`.
@Sym d
...
# ## Assembly
# Note the compilation may take a while (~6min on the developer's machine), which will be optimized in the future release.
