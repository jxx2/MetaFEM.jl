...
# The function `initialize_Definitions` clears the previous defined environment to prevent any interference if we run multiple scripts in sequence.
...
#
# Here we read the mesh from file 3D\_COMSOL\_Mesh.mphtxt generated by COMSOL and should be copied to the same folder as your script.  
# The vertices coordinates are divided by 100 here because the source mesh of our pikachu has the length scale in cm.
...
# * Only the variables which are actually in use (in a weak-form assigned to a workpiece) will be allocated. Also, MetaFEM has the default variable declarations:
# ```julia
...
# ## Assembly
# After defining physics, we need to collect, classify and number all the variables each workpiece. We may defines explicitly the maximum spatial differential order per workpiece, i.e., weak-form spatial derivatives order higher than "explicit\_max\_sd\_order" will not be stored to save memory. 
...
# CGS/CGS2 (oscillate a lot before convergence but efficient for some problems, e.g., incompressible flow) and TFQMR (milder than CGS).
