...
fixed_bg_ID = add_BoundaryGroup(wp_ID, facet_IDs[.~(is_left .| is_right)]; fem_domain = fem_domain)
inflow_bg_ID = add_BoundaryGroup(wp_ID, facet_IDs[is_left]; fem_domain = fem_domain)
outflow_bg_ID = add_BoundaryGroup(wp_ID, facet_IDs[is_right]; fem_domain = fem_domain)
# Define physics
Δx = 0.02 # r = 0.1
ρ = 1e3
μ = 1.
...
@time begin
    ...
    assign_BoundaryGroup_WeakForm(wp_ID, inflow_bg_ID, WF_boundary_inflow; fem_domain = fem_domain)
    assign_BoundaryGroup_WeakForm(wp_ID, outflow_bg_ID, WF_boundary_outflow; fem_domain = fem_domain)
