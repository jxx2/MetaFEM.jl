...
d2_fix_bg_ID = add_BoundaryGroup(wp_ID, facet_IDs_front; fem_domain = fem_domain) #left fixed
d3_fix_bg_ID = add_BoundaryGroup(wp_ID, facet_IDs_bottom; fem_domain = fem_domain) #left fixed

free_bg_ID = add_BoundaryGroup(wp_ID, vcat(facet_IDs_right, facet_IDs_top); fem_domain = fem_domain) #bot & right free
loaded_bg_ID = add_BoundaryGroup(wp_ID, facet_IDs_back; fem_domain = fem_domain) #top loadeds
...
assign_BoundaryGroup_WeakForm(wp_ID, d3_fix_bg_ID, WF_d3_fixed_bdy; fem_domain = fem_domain)
