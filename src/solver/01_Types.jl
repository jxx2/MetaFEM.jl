...
mutable struct FEM_Physics
    ...
    boundarygroups::Vector{CuVector{FEM_Int}} #each boundary group is a set of ref_edge_IDs
...
function add_BoundaryGroup(ID::Integer, bdy_ref_edge_IDs::CuVector; fem_domain::FEM_Domain = fem_domain, target::Symbol = :WP)
    if target == :WP
        boundarygroups = fem_domain.workpieces[ID].physics.boundarygroups
        msg = string("BoundaryGroup ", length(boundarygroups) + 1, " added to Workpiece ", ID, " !")
    elseif target == :Tool
        boundarygroups = fem_domain.tools[ID].boundarygroups
        msg = string("BoundaryGroup ", length(boundarygroups) + 1, " added to Tool ", ID, " !")
    end
    push!(boundarygroups, bdy_ref_edge_IDs)
    bg_ID = boundarygroups |> length |> FEM_Int
    println(msg)
    return bg_ID
end

function assign_BoundaryGroup_WeakForm(wp_ID::Integer, bg_ID::Integer, this_term::SymbolicTerm; fem_domain::FEM_Domain)
...
assign_BoundaryGroup_WeakForm(wp_ID::FEM_Int, bg_ID::FEM_Int, this_term::FEM_Float; fem_domain::FEM_Domain) = this_term == 0 ? this_term : error()
