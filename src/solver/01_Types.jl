...
mutable struct FEM_Physics{ArrayType}
    ...
    boundary_weakform_pairs::Dict{FEM_Int, Vector{Symbolic_BilinearForm}} #bg_ID, wf
    domain_weakform::Vector{Symbolic_BilinearForm}
    FEM_Physics(::Type{ArrayType}) where {ArrayType} = new{ArrayType}(Dict{Symbol, FEM_Float}(), Symbol[], AbstractArray{FEM_Int, 1}[], Dict{FEM_Int, Vector{Symbolic_BilinearForm}}())
end
...
mutable struct FEM_Domain{ArrayType} #workgroup
    ...
    FEM_Domain(::Type{ArrayType} = DEFAULT_ARRAYINFO._type; dim::Integer, dissipative::Bool = true) where {ArrayType} = new{ArrayType}(dim, TensorTable(dim), WorkPiece{ArrayType}[], Tool{ArrayType}[], GeneralAlpha(; dissipative = dissipative), GlobalField(ArrayType)) # need to rewrite
end
...
"""
function assign_Boundary_WeakForm!(wp_ID::Integer, bg_ID::Integer, this_term; fem_domain::FEM_Domain)
