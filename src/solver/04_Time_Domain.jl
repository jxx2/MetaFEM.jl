...
GeneralAlpha() = GeneralAlpha(FEM_Float(0.), (1., 1., 1.) .|> FEM_Float, (0.5, 0.5) .|> FEM_Float, FEM_Float[], FEM_Float[])
...
function update_OneStep(time_discretization::GeneralAlpha; max_iter::Integer = 4, fem_domain::FEM_Domain)
    ...
    while true
        ...
        fem_domain.K_nonlinear_func(time_discretization; fem_domain = fem_domain)
        ...
        delta_x = fem_domain.linear_solver(globalfield)
