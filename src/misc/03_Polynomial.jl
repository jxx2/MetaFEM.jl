...
@inline Base.:+(num::Number, p1::Polynomial{dim}) where dim = p1 + num
...
@inline function Base.:-(p1::Polynomial{dim}) where dim
    p_ans = Polynomial(p1)
    p_ans.factors .*= -1
    return p_ans
end
@inline Base.:-(p1::Polynomial{dim}, num::Number) where {dim} = p1 + (-num)
@inline Base.:-(num::Number, p1::Polynomial{dim}) where {dim} = -p1 + num
@inline Base.:-(p1::Polynomial{dim}, p2::Polynomial{dim}) where {dim} = p1 + (-p2)
...
@inline Base.:*(num::Number, p1::Polynomial{dim}) where dim = p1 * num
@inline Base.:/(p1::Polynomial{dim}, num::Number) where dim = p1 * (1/num)
...
Base.@pure function Base.:^(p1::Polynomial{dim}, num::Integer) where dim #can be rewrite in to a ^ 4 = (a^2)^2 ...
...
Base.@pure function generic_CPU_Integration_Lookup(p1::Polynomial{dim}, moment_of_inertia) where dim
    sum = 0
    for (this_factor, this_order) in zip(p1.factors, p1.orders)
...
Base.@pure function evaluate_Polynomial(p1::Polynomial{dim}, pos::Tuple) where dim
    sum = 0
    for (this_factor, this_order) in zip(p1.factors, p1.orders)
