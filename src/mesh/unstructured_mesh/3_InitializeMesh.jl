function allocate_Basic_WP_Mesh_2D(wp::WorkPiece, this_space::Classical_Discretization)
    ...
    for (bg_index, boundarygroup) in enumerate(wp.physics.boundarygroups)
        bdyIDs = allocate_by_length!(facets, length(boundarygroup))
        bg_fIDs[bg_index] = bdyIDs
        segment_facet_IDs[boundarygroup] .= bdyIDs
    end
...
function allocate_Basic_WP_Mesh_3D(wp::WorkPiece, this_space::Classical_Discretization)
    ...
    for (bg_index, boundarygroup) in enumerate(wp.physics.boundarygroups)
        bdyIDs = allocate_by_length!(facets, length(boundarygroup))
        bg_fIDs[bg_index] = bdyIDs
        face_facet_IDs[boundarygroup] .= bdyIDs
    end
